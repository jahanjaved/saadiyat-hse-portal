...
async function generateWord() {
  ...
  setStatus("Generating final updated ALDAR Word file...");
  ...
  const titleTable = new Table({ width:{size:100,type:WidthType.PERCENTAGE}, rows:[new TableRow({children:[
    new TableCell({borders:noneBorder,width:{size:16,type:WidthType.PERCENTAGE},children:[new Paragraph({children:[new ImageRun({data:logoBuffer,transformation:{width:52,height:52}})]})]}),
  ...
  saveAs(blob, `OSH_Inspection_Report_Final_${safeRef || "report"}.docx`);
