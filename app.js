...
function addObservation(seed = null) {
  ...
  fillSelect($(".actionOwners", card), [], ""); // harmless if absent
...
async function generateWord() {
  ...
  setStatus("Generating FINAL PERFECT ALDAR v3 Word file...");
  ...
  const titleTable = new Table({ width:{size:100,type:WidthType.PERCENTAGE}, rows:[new TableRow({children:[
    new TableCell({borders:noneBorder,width:{size:16,type:WidthType.PERCENTAGE},children:EMBEDDED_LOGO_BASE64 ? [new Paragraph({children:[new ImageRun({data:logoBuffer,transformation:{width:52,height:52}})]})] : [docParagraph(" ")]}),
  ...
  saveAs(blob, `Noor_FINAL_PERFECT_ALDAR_v3_${safeRef || "report"}.docx`);
  setStatus("FINAL PERFECT ALDAR v3 generated successfully.", "success");
}
