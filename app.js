...
function detectManualRule(text, selectedCategory=''){
  ...
  if(hasWord(t,['scaffold','scaffolding','mobile tower','outrigger','green tag','red tag','plank','platform','ladder','a-frame','a frame'])) return 'scaffold';
...
function issueToObservation(issue){
  ...
  if(/ladder|single ladder|a-frame|a frame/.test(i)) return 'Ladder was being used in an unsafe manner / as a working platform, creating risk of fall from height.';
...
function buildImmediateFromRule(rule, text){
  const base={
    ...
    scaffold:['Unsafe scaffold / ladder use was stopped immediately and access to the affected arrangement was restricted.','Workers were removed from the unsafe platform and the scaffolding / supervision team was instructed to rectify the condition.','The scaffold was instructed to be inspected and released only by a competent scaffolding inspector before reuse.'],
...
function buildActionsFromRule(rule, text){
  ...
  if(/scaffold|platform|outrigger|ladder/.test(t)){
    ...
    if(/ladder/.test(t)) a.push('Ladders shall not be used as working platforms and shall only be used for safe access / egress where suitable and properly secured.');
  }
...
function actionVerbForPoint(point){
  ...
  if(/scaffold|platform|outrigger|ladder/.test(t)) return 'rectify and inspect the scaffold / access platform by a competent person before use';
...
function specificObservationSentence(point,areaText=''){
  ...
  if(/scaffold|platform|outrigger|ladder/.test(t)) return prefix+'scaffold / access platform arrangement was not maintained in safe and compliant condition'+areaText+', creating fall from height and instability hazards.';
...
function buildSpecificManualReport(text, selectedCategory='', area=''){
  ...
  const immediate=['The observation was immediately communicated to the responsible site supervision and contractor HSE team for action.'];
  pts.forEach(pt=>immediate.push('Contractor was instructed to '+actionVerbForPoint(pt)+' without delay.'));
  immediate.push('The affected area / activity shall remain under supervision until the required controls are implemented and verified.');
  const actions=[];
  pts.forEach(pt=>actions.push('The contractor shall '+actionVerbForPoint(pt)+' and submit closeout evidence for verification.'));
  actions.push('The contractor shall inspect similar welfare / work areas across the project and rectify any repeated deficiencies.');
  actions.push('Responsible engineers, supervisors and HSE personnel shall conduct routine inspections to ensure the implemented controls are maintained.');
  actions.push('Lessons learned shall be communicated to relevant supervisors and workforce to prevent recurrence.');
...
function smartRuleFor(text,selectedCategory=''){
  ...
  if(hasAny(t,['scaffold','scaffolding','mobile scaffold','ladder','outrigger','green tag','red tag','plank','platform'])) return 'scaffold';
  ...
  if(cat.includes('scaffold')||cat.includes('ladder')) return 'scaffold';
