...
function fillChips(container, items, textarea, replace=false, formatter=null) {
  container.innerHTML = "";
  const seen = new Set();
  const cleanItems = [];
  items.forEach(item => {
  ...
  cleanItems.forEach((item, index) => {
    ...
    chip.addEventListener("click", () => {
      const finalText = formatter ? formatter(item) : item;
      if (replace && !textarea.value.trim()) setTextarea(textarea, finalText);
      else appendToTextarea(textarea, finalText);
    });
...
function updateAreaPreview(card) {
  ...
  $(".areaPreview", card).textContent = `Area: Package ${pkg || "-"} - ${cluster || "-"}`;
}
...
function applyPresetToCard(card, replaceText = true) {
  ...
  fillChips($(".descriptionChips", card), preset.description || [], $(".observationDescription", card), true, value => humanizeObservationDescription(value, key));
  fillChips($(".gapChips", card), gapOptions, $(".topGaps", card), false, humanizeGapText);
  fillChips($(".immediateChips", card), immediateOptions, $(".immediateAction", card), false, humanizeImmediateAction);
  fillChips($(".preventiveChips", card), preventiveOptions, $(".preventiveAction", card), false, humanizePreventiveAction);

  if (replaceText) {
    setTextarea($(".observationDescription", card), humanizeObservationDescription((preset.description || []).join("\n"), key));
    const firstThree = gapOptions.slice(0, 3).map((g, i) => `${i+1}. ${g}`);
    setTextarea($(".topGaps", card), humanizeGapText(firstThree.join("\n")));
    setTextarea($(".immediateAction", card), humanizeImmediateAction((preset.immediate || []).join("\n")));
...
function addObservation(seed = null) {
  ...
  $(".descriptionPresetSelect", live).addEventListener("change", e => setTextarea($(".observationDescription", live), humanizeObservationDescription(e.target.value, $(".observationCategory", live).value)));
  $(".gapPresetSelect", live).addEventListener("change", e => appendToTextarea($(".topGaps", live), humanizeGapText(e.target.value)));
  $(".immediatePresetSelect", live).addEventListener("change", e => appendToTextarea($(".immediateAction", live), humanizeImmediateAction(e.target.value)));
  $(".preventivePresetSelect", live).addEventListener("change", e => appendToTextarea($(".preventiveAction", live), humanizePreventiveAction(e.target.value)));
...
function getObservations() {
  return $$(".obs-card").map((card, index) => ({
    ...
    topGaps: humanizeGapText($(".topGaps", card).value.trim()),
    observationDescription: humanizeObservationDescription($(".observationDescription", card).value.trim(), $(".observationCategory", card).value),
    preventiveAction: humanizePreventiveAction($(".preventiveAction", card).value.trim()),
    rootCause: humanizeRootCause($(".rootCause", card).value),
    dateClosed: $(".dateClosed", card).value,
    immediateAction: humanizeImmediateAction($(".immediateAction", card).value.trim()),
...
function dedupeLines(lines) {
  ...
  const seen = new Set();
  lines.forEach(line => {
    const key = normalizeHumanText(line).toLowerCase();
    if (key && !seen.has(key)) {
      seen.add(key);
      out.push(line.trim());
    }
...
const HUMAN_GAP_PATTERNS = [
  { match: /leadership failure/i, detail: "A clear gap was observed in leadership visibility and site-level control, as the responsible management team did not demonstrate the required level of intervention to prevent the unsafe condition from developing at the work front." },
  { match: /poor supervision|inadequate superv/i, detail: "The supervision arrangement at the affected area was not effective, and the responsible supervisors did not adequately monitor the activity to ensure that the workforce complied with the approved safety requirements." },
  { match: /poor subcontractor control|sub-?contractor/i, detail: "Control over the subcontractor workforce was found to be weak, as the activity was allowed to continue without sufficient coordination, close follow-up, or verification by the main contractor’s responsible team." },
  { match: /ptw failure|permit/i, detail: "A deficiency was noted in the implementation and field verification of the permit-to-work system, as the required controls were either not properly established or were not maintained during the execution of the activity." },
  { match: /planning|coordination|simops/i, detail: "The activity was not planned and coordinated to the required standard, resulting in avoidable interface issues, weak work sequencing, and insufficient consideration of the site constraints before execution." },
  { match: /inspection|monitoring|compliance/i, detail: "Routine inspection and monitoring were not effective enough to identify and close the unsafe condition at an early stage, which allowed the deficiency to remain visible during the inspection." },
  { match: /training|awareness|compet/i, detail: "The observed condition also indicated a weakness in competency and workforce awareness, as the personnel involved did not demonstrate the expected understanding of the project safety requirements." }
];
const HUMAN_IMMEDIATE_PATTERNS = [
  { match: /stopped|suspended|cancelled/i, detail: "The unsafe activity was immediately stopped at the point of observation and the affected work front was placed under control until the required corrective measures could be implemented and verified by the responsible team." },
  { match: /called the area engineer|called/i, detail: "The responsible engineer and supervision team were immediately called to the location, briefed on the unsafe condition, and instructed to take urgent corrective action without delay." },
  { match: /barricad|isolat/i, detail: "The affected area was isolated and physically controlled to prevent further exposure of personnel until suitable protective measures were restored and verified." },
  { match: /commended|leadership/i, detail: "Positive performance was acknowledged on site and the responsible team was advised to maintain the same standard through continued supervision, housekeeping control, and monitoring of subcontractor activities." }
];
const HUMAN_PREVENTIVE_PATTERNS = [
  { match: /ptw must be controlled by the main contractor|ptw shall remain under the control|permit compliance/i, detail: "Permit-to-work control shall remain fully under the authority of the main contractor. No activity shall proceed unless the permit is valid, the work front has been physically verified, and all required precautions are fully in place. The responsible supervision team shall closely monitor the activity during execution to ensure compliance with the approved permit conditions and project OSH requirements." },
  { match: /close supervision of sub-?contractors|provide competent superv|strict supervision|close competent supervision/i, detail: "Close and competent supervision shall be maintained by the responsible construction and HSE team to monitor all subcontractor activities, verify compliance with the approved method statement and risk assessment, and intervene immediately whenever any unsafe act, unsafe condition, or procedural deviation is identified on site." },
  { match: /conduct coordination meetings|pre-?plan|look-?ahead planning|simos|simops/i, detail: "The activities shall be properly pre-planned through regular coordination meetings and short-term look-ahead planning so that work interfaces, simultaneous operations, access arrangements, and resource requirements are identified in advance. This shall help eliminate conflicts between trades and ensure that the work is executed in a safe and controlled manner." },
  { match: /waste collection|housekeeping|temporary storage area for waste|waste management/i, detail: "Adequate housekeeping arrangements and waste collection facilities shall be provided and maintained at all times. Waste shall be removed on a routine basis, temporary storage locations shall be clearly designated, and all work areas shall be kept clean and unobstructed to prevent slip, trip, fire, and access-related hazards." },
  { match: /training and awareness|regular training sessions|toolbox talks/i, detail: "Regular training and awareness sessions shall be conducted for supervisors, operatives, and subcontractor teams to reinforce project OSH requirements, hazard identification, permit controls, and the safe execution of high-risk activities. The effectiveness of these sessions shall be checked through follow-up inspections and supervisory monitoring." },
  { match: /scaffold|scaffolding inspector/i, detail: "All scaffolding and temporary access arrangements shall be inspected by a competent scaffolding inspector at the required frequency, and no scaffold shall be used unless it fully complies with the approved design, inspection requirements, and tagging system applicable to the project." },
  { match: /distribution board inspection|db inspection|electrical/i, detail: "Distribution boards, temporary electrical systems, and associated tools and cables shall be routinely inspected by competent persons to confirm that the equipment remains safe, compliant, and suitable for continued use within the work area." },
  { match: /safe and proper access platforms|working platforms|access scaffolds|mobile platforms|safe access/i, detail: "Safe and proper access arrangements, including working platforms, access scaffolds, ladders, and mobile platforms where applicable, shall be provided and maintained in accordance with the approved method statement and risk assessment. These arrangements shall be inspected regularly to ensure stability, suitability, and safe access for the workforce." },
  { match: /competency assessment|competenc/i, detail: "A formal competency assessment shall be carried out for the personnel involved in the activity to verify that only trained, experienced, and authorized workers and supervisors are assigned to the task. Any identified competency gap shall be addressed through retraining, reassessment, and closer supervision." },
  { match: /preventive maintenance|period monitoring|monitoring/i, detail: "Periodic monitoring and preventive maintenance shall be implemented to ensure that plant, tools, temporary systems, and safety arrangements remain effective and do not deteriorate during the execution of the work." },
  { match: /non-performing subcontractors shall not be paid|non-performing construction team shall be replaced/i, detail: "The performance of subcontractors and construction teams shall be closely reviewed by project management, and any repeated failure to comply with project OSH requirements shall be addressed through formal corrective measures, including suspension of work, replacement of non-performing personnel, and enforcement through the contractor’s management system." },
  { match: /hot work permit|fire watcher|fire watch|smoking/i, detail: "Hot work activities shall be carried out only under a valid hot work permit, with continuous supervision, appropriate firefighting equipment, and a dedicated fire watch where required. Smoking shall be restricted to designated areas only, and combustible materials shall be controlled or removed from the work area to prevent fire incidents." }
];
function expandByPatterns(line, patterns, fallback) {
  const clean = normalizeHumanText(line).replace(/^\d+[\.\)]\s*/, "").trim();
  if (!clean) return "";
...
function humanizeGapText(value) {
  const lines = dedupeLines(splitLines(value).map(line => expandByPatterns(line, HUMAN_GAP_PATTERNS, clean => `${toSentenceCase(clean)} was identified as a contributing gap during the inspection and requires close management attention to prevent recurrence of the unsafe condition.`)));
  return lines.join("\n");
}
function humanizeImmediateAction(value) {
  const lines = dedupeLines(splitLines(value).map(line => expandByPatterns(line, HUMAN_IMMEDIATE_PATTERNS, clean => `${toSentenceCase(clean)} Immediate control measures were communicated on site and the responsible team was instructed to rectify the unsafe condition without delay.`)));
  return lines.join("\n");
}
function humanizePreventiveAction(value) {
  const lines = dedupeLines(splitLines(value).map(line => expandByPatterns(line, HUMAN_PREVENTIVE_PATTERNS, clean => `${toSentenceCase(clean)} This requirement shall be implemented by the responsible construction and supervision team, verified through routine inspections, and sustained through continuous monitoring to ensure full compliance with project OSH requirements.`)));
  return lines.join("\n\n");
}
function humanizeObservationDescription(value, category) {
  const lines = dedupeLines(splitLines(value).map(line => {
    const clean = normalizeHumanText(line).replace(/^[A-Z]\.\s*/, "");
    if (!clean) return "";
    const cat = category ? ` related to ${category.toLowerCase()}` : "";
    if (/observed|it has been observed/i.test(clean)) {
      return toSentenceCase(clean);
    }
    return `It was observed during the inspection that ${clean.charAt(0).toLowerCase()}${clean.slice(1)}. This condition represents a safety concern${cat} and requires timely corrective and preventive action to avoid recurrence.`;
  }));
  return lines.join("\n");
}
function humanizeRootCause(value) {
  const clean = normalizeHumanText(value);
  if (!clean) return "";
  return expandByPatterns(clean, HUMAN_GAP_PATTERNS, text => `${toSentenceCase(text)} was considered the most likely root cause based on the condition observed at the work front and the level of control demonstrated during the inspection.`);
}
...
async function generateWord() {
  ...
  data.observations.forEach(obs => {
    ...
    if (obs.topGaps) { addLine("Top 3 Gaps Observed:", true); splitLines(humanizeGapText(obs.topGaps)).forEach(line => addLine(line, false)); }
    addLine("Observation Description:", true);
    splitLines(humanizeObservationDescription(obs.observationDescription, obs.observationCategory)).forEach(line => addLine(line, false));
    if (obs.immediateAction) { addLine("Immediate Action Taken:", true); splitLines(humanizeImmediateAction(obs.immediateAction)).forEach(line => addLine(line, false)); }
